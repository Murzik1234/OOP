...
public class SerializeTest extends junit.framework.TestCase {
    ...
    public void testTxtSerialize() throws IOException {
        ...
        productsList.setAll(new SimpleProduct("Соль", 12, 12, "Варить", 6));
        Dish dish = new Dish("Суп", productsList);
        ...
        assertEquals("Error serialization : wrong dish name",mainDishesList.get(0).getName(), "Суп");
        assertEquals("Error serialization : wrong product list size",mainDishesList.get(0).getProductsList().size(), 1);
        assertEquals("Error serialization : wrong product name",mainDishesList.get(0).getProductsList().get(0).getName(), "Соль");
